...
public static class ProcessModuleAuditor
{
    ...
    public static AuditCheckResult Audit(SteamLayout layout, AuditReport report, CancellationToken cancellationToken)
    {
                ...
                try
                {
                    ...
                    report.Findings.Add(new AuditFinding
                    {
                        ...
                        Meaning = unsignedOrInvalid
                            ...
                            : "创意工坊或 Wallpaper 项目通常不应注册独立常驻程序。有效签名会降低风险，但仍需核对来源。",
                        Recommendation = "先断网并退出 Steam，不要付款或登录。导出只读取证包后，用专业杀毒软件扫描该路径及全盘。",
                        Target = path,
                        ...
                        foreach (ProcessModule module in process.Modules)
                        {
                            ...
                            if (!inSteam && !inWindows && userWritableLocation)
                            {
                                ...
                                report.Findings.Add(new AuditFinding
                                {
                                    ...
                                    Meaning = "覆盖层、无障碍工具或客户端插件也可能这样工作，这不能单独作为中毒结论。",
                                    Recommendation = "核对它是否来自你主动安装的软件。无法确认时，把路径交给专业杀毒软件复核。",
                                    Target = path,
        ...
        AuditLevel level = count > 0
            ...
            : AuditLevel.Passed;
        if (inaccessible > 0) report.CoverageNotes.Add($"有 {inaccessible} 个 Steam 相关进程无法枚举模块，通常与进程权限或退出时机有关。");
        return new AuditCheckResult
        {
            ...
            Summary = relatedProcessCount == 0
                ? "Steam 与 Wallpaper 内容程序当前未运行，静态检查不受影响，动态项已跳过。"
                : count == 0
...
public static class RegistryPersistenceAuditor
{
    ...
    public static AuditCheckResult Audit(SteamLayout layout, AuditReport report)
    {
        ...
        return new AuditCheckResult
        {
            ...
            Name = "Steam 相关启动项",
            Level = level,
            Summary = count == 0 ? "未发现劫持 Steam 进程的 Run、IFEO 或 SilentProcessExit 配置。" : $"发现 {count} 项 Steam 相关持久化配置需要核对。",
            EvidenceCount = count
        };
    }
            ...
            foreach (string valueName in key.GetValueNames())
            {
                ...
                report.Findings.Add(new AuditFinding
                {
                    ...
                    Meaning = wallpaperStartup
                        ? "创意工坊内容通常不应建立 Windows 自启动，这与已观察到的假红信持久化手法一致。"
                        : scriptHost
                            ? "启动项借助脚本宿主或系统加载器触发，和普通 Steam -silent 自启动不同。"
                            : "客户端插件也可能创建此项，需要由用户确认来源。",
                    Recommendation = "不要由本工具删除。先在任务管理器启动应用或专业杀毒软件中核对，不认识时按完整处置链处理。",
    ...
    private static void InspectIfeo(RegistryView view, string executable, AuditReport report)
    {
        ...
        try
        {
            ...
            report.Findings.Add(new AuditFinding
            {
                ...
                Meaning = "开发调试工具可能合法使用 IFEO，但普通 Steam 安装不需要它，恶意程序可借此劫持启动。",
                Recommendation = "记录该值并交给专业杀毒软件处理。如果这是你主动配置的调试器，可在报告中注明。",
    ...
    private static void InspectSilentExit(RegistryView view, string executable, AuditReport report)
    {
        ...
        try
        {
            ...
            report.Findings.Add(new AuditFinding
            {
                ...
                Recommendation = "不要手工猜测删除范围，将键路径与监控命令交给专业杀毒软件复核。",
    ...
    public static AuditCheckResult Audit(AuditReport report)
    {
        try
        {
            ...
            report.Findings.Add(new AuditFinding
            {
                ...
                Meaning = local
                    ? "本地代理常见于 Clash 等用户主动安装的网络工具，本身不代表异常，也不参与本工具的风险结论。"
                    : "企业网络、VPN 和调试工具也会配置代理，仅凭代理存在不能判断恶意。",
                Recommendation = "如果这是你主动配置的网络工具，无需处理。如果完全不认识该配置，请交给专业杀毒软件和网络设置一起核对。",
            ...
            return new AuditCheckResult
            {
                ...
                Name = "系统代理（仅提示）",
                Level = AuditLevel.Information,
                Summary = local ? "发现本地代理配置，它本身不构成风险结论。" : "发现代理/PAC 配置，已客观列出供核对。",
                EvidenceCount = 1
            };
        }
        catch (Exception ex) when (ex is UnauthorizedAccessException or System.Security.SecurityException or IOException)
    ...
    public static AuditCheckResult Observe(SteamLayout layout, AuditReport report, CancellationToken cancellationToken)
    {
        ...
        if (projects.Count > 0)
        {
            ...
            report.Findings.Add(new AuditFinding
            {
                ...
                Meaning = "应用程序壁纸能够包含可执行内容，但“存在”绝不等于“有毒”。本项只帮助定位来源，不参与篡改结论。",
                Recommendation = "若专业杀毒软件报出具体文件，再根据工坊 ID 核对来源，不要批量删除正常壁纸。",
